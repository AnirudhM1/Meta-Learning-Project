...
agent = Agent(env, psi, fqn, max_walks=1000)
...
for epoch in range(TOTAL_EPOCHS):
    ...
    for _ in range(INITIAL_GATHERING_LENGTH):
        ...
        agent.psi_replay.push(states, actions, 1)

    for _ in range(2*INITIAL_GATHERING_LENGTH):
        state = env.reset()
        done = False
        step = 0
    ...
    torch.save(psi.state_dict(), 'psi.pt')
    torch.save(fqn.state_dict(), 'fqn.pt')